...
class massTranslateProduct extends \Magento\Backend\App\Action
{
    ...
    protected function setTranslationJob(array $ids, $storeId)
    {
        ...
        if ($cronTranslate2->count() > 0) {
            foreach ($cronTranslate2 as $abortCron2) {
                $cronDataUpdate = $this->_masstranslateinAllstoreFactory->create();
                $abortCron = $cronDataUpdate->load($abortCron2->getId())->setUpdateCronDate($this->_date->gmtDate())->setIsAbort(1)->setStatus('abort')->save();
            }
        }
        ...
        if ($cronTranslate3->count() > 0) {
            foreach ($cronTranslate3 as $abortCron3) {
                $cronDataUpdate = $this->_masstranslateNewlyAddedProductsFactory->create();
                $abortCron = $cronDataUpdate->load($abortCron3->getId())->setUpdateCronDate($this->_date->gmtDate())->setIsAbort(1)->setStatus('abort')->save();
            }
        }
        ...
        try {
            ...
            $this->messageManager->addSuccess(__('Cron Process Registered for the storeview : '. $this->_storeManager->getStore($storeId)->getName().' | Make sure you have filtered that storeview in you wanted to translation. | Please enable log to view any error occured'));
