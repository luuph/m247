...
use Biztech\Translator\Helper\CheckMassTranslateInAllStoreviewLogger\Logger;
...
class CheckTranslateMassProductsInAllstore
{
    ...
    public function execute()
    {
        ...
        if ($this->_date->gmtDate('d-m-Y') == date('d-m-Y', strtotime($_logCron->getCronDate()!=null?$_logCron->getCronDate():""))) {
            ...
            if ($_logCron->getCronJobCode() == 'masstranslateinallstorecron') {
                $this->_logger->info('Translator Check Start');

                $_cronModel = $this->_masstranslateinAllstoreFactory->create();
                ...
                if ($_cronProducts->count() > 0) {
                    ...
                    if (is_array($_pTranslate) && !empty($_pTranslate)) {
                        $_checkCronProducts = $this->_masstranslateinAllstoreFactory->create()->getCollection()->addFieldToFilter('store_ids', ['eq' => json_encode($_pTranslate)])->addFieldToFilter('status', 'pending');
                        if ($_checkCronProducts->count() == 0) {
                            $this->_masstranslateinAllstoreFactory->create()->load($_cronProducts->getFirstItem()->getId())->setStatus('abort1')->setIsAbort(1)->save();
                            ...
                            try {
                                $_cronUpdate = $this->_masstranslateinAllstoreFactory->create();
                        ...
                        if (empty($_pTranslate)) {
                                ...
                                $this->_masstranslateinAllstoreFactory->create()->load($_cronProducts->getFirstItem()->getId())->setStatus('success')->save();
                                $cronMail_for = $this->_bizHelper->cronMailFor();
                            if (in_array('translateinmultiplestore', $cronMail_for)) {
                                $this->_bizHelper->sendEmailNotification("Bulk Product Translation in Multiple store", $_cronProducts->getFirstItem()->getId(), $_logCron->getRemainLimit());
                            }
                        }
                    }
                }
