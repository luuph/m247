...
use Biztech\Translator\Helper\MassTranslateInAllStoreviewLogger\Logger;
use Biztech\Translator\Helper\Data as BizHelper;
use Biztech\Translator\Helper\Translator;
use Biztech\Translator\Model\MasstranslateinAllstore;
 ...
 */
class TranslateMassProductsInAllstore
{
    protected $_date;
    ...
    protected $_masstranslateinAllstore;
    protected $_logCron;
     ...
     * @param MasstranslateinAllstore                   $_masstranslateinAllstore
    ...
    public function __construct(
        ...
        MasstranslateinAllstore $_masstranslateinAllstore,
        ...
        $this->_masstranslateinAllstore = $_masstranslateinAllstore;
    ...
    public function execute()
    {
        ...
        $jobCode = \Biztech\Translator\Model\MasstranslateinAllstore::MASS_TRANSLATE_IN_ALLSTORE_CRON_JOB_CODE;
        ...
        $_cronProducts = $this->_masstranslateinAllstore->getCollection()->addFieldToFilter('status', 'pending');
        $characterLimit = (int)$this->_bizHelper->getConfigValue('translator/general/google_daily_limit') - $_charCutLimit;
        if (count($_cronProducts) && !$this->_bizHelper->translateInAllStoreviewEnable()) {
            $this->_logger->info('Products Translation in Multiple Storeview is disabled, to use  enable [Mass Products Translation in Multiple Storeview] configuration');
            return;
        }
        foreach ($_cronProducts as $cronProductData) {
            ...
            foreach (array_chunk($_productIds, $batchSize) as $productId) {
                ...
                if ($characterLimit > 0) {
                    ...
                    $this->batchproductTranslate($_storeIds, $productId, $characterLimit, $jobCode, $cronProductData->getId());
                }
            }
        }
    ...
    protected function batchproductTranslate($storeIds, $batchProducts, &$characterLimit, $jobCode, $cronId = null)
    {
        ...
        foreach ($storeIds as $storeId) {
            ...
            $cron_data1 = $this->_masstranslateinAllstore->load($cronId);
            ...
            foreach ($batchProducts as $batchProduct) {
                ...
                if ($i == 10) {
                    if ($cronId) {
                        $_checkCron = $this->_masstranslateinAllstore->load($cronId);
            ...
            $success = true;
            $cron_data = $this->_masstranslateinAllstore->load($cronId);
