...
class Stopcronremoveobserver extends ProcessCronQueueObserver
{
    /**#@+
     * Cache key values
     */
    ...
    protected function _runJob($scheduledTime, $currentTime, $jobConfig, $schedule, $groupId)
    {
        ...
        if (!is_callable($callback)) {
            ...
            $date = new \DateTime();
            $date->setTimestamp($this->dateTime->gmtTimestamp());
            $schedule->setExecutedAt($date->format('Y-m-d H:i:s'))->save();
       ...
       $date->setTimestamp($this->dateTime->gmtTimestamp());
        $schedule->setStatus(Schedule::STATUS_SUCCESS)->setFinishedAt($date->format('Y-m-d H:i:s'));
    ...
    protected function saveSchedule($jobCode, $cronExpression, $timeInterval, $exists)
    {
        ...
        for ($time = $currentTime; $time < $timeAhead; $time += self::SECONDS_IN_MINUTE) {
            ...
            $scheduledAt=$date->format('Y-m-d H:i:00');

            //$scheduledAt=date('Y-m-d H:i:s', $time);
    ...
    protected function generateSchedule($jobCode, $cronExpression, $time)
    {
        ...
        $schedule = $this->_scheduleFactory->create()
            ...
            ->setScheduledAt($date->format('Y-m-d H:i:s'));
    ...
    {
        //$this->logger->info('=======================================================');
        //$this->logger->info('  Start Cron ' . date('d-m-Y H:i:s', time()));
        
        $jobs = $this->getJobs();
        foreach ($jobs[$groupId] as $jobCode => $jobConfig) {
            //$this->logger->info('  Job Code ' . $jobCode);
            if (!$this->getCronExpression($jobConfig)) {
                
                if ($jobCode=='bizgridcrontranslation') {
