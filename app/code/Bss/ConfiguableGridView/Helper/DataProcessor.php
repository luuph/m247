...
class DataProcessor extends \Magento\ConfigurableProduct\Helper\Data
{
    ...
    /**
     * @var \Bss\ConfiguableGridView\Model\MultiInventorySource\MsiObjectManagement
     */
    protected $msiObjectManagement;
    ...
    public function __construct(
        ...
        \Bss\ConfiguableGridView\Model\MultiInventorySource\MsiObjectManagement $msiObjectManagement
    ) {
        parent::__construct(
            $imageHelper,
            $imageUrlBuilder
        ...
        $this->msiObjectManagement = $msiObjectManagement;
    }
    ...
    public function getConfigurableGridViewData($currentProduct, $label = null)
    {
        if(!$this->assocProductData) {
            ...
            $attrLabel = [];
            $sort = false;
            // Parent product quantity increment if exist
            $parentExtensionAttributes = $currentProduct->getExtensionAttributes();
            if($parentExtensionAttributes){
                $stockItem = $parentExtensionAttributes->getStockItem();
                $parentQtyIncrement = $stockItem->getQtyIncrements() ? $stockItem->getQtyIncrements() : 1;
            } else {
                $parentQtyIncrement = 1;
            }
            foreach ($this->getAllowProducts($currentProduct) as $product) {
                ...
                if ($this->moduleManager->isEnabled('Magento_Inventory')) {
                    ...
                    $stockItemData = $this->msiObjectManagement->getStockItemDataInterface()->execute($product->getSku(), $stockId);
                    $stockStatus = ($salableQty > 0 || ($backOrder != self::DISABLE_BACKORDER && $stockItemData !== null && $stockItemData['is_salable']) != '0') ? self::IS_IN_STOCK : self::IS_OUT_STOCK;
                ...
                // Child product quantity increment
                $qtyIncrement = $stockItem->getQtyIncrements() ? $stockItem->getQtyIncrements() : 1;
                if($qtyIncrement) {
                    // If product quantity increment is greater than its parent, set equal as its parent
                    if($qtyIncrement > $parentQtyIncrement) {
                        $data['qty_increment'] = $qtyIncrement;
                    } else {
                        $data['qty_increment'] = $parentQtyIncrement;
                    }
                }
