...
class DescriptionOption extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    /**
     * Class constructor
     *
    ...
    {
        $this->_init('bss_custom_option_description', 'id');
    }
     ...
     */
    public function saveDescription($result)
    {
        $connection = $this->getConnection();
        $titleTableName = $this->getTable('bss_custom_option_description');
        foreach ([Store::DEFAULT_STORE_ID, $result->getStoreId()] as $storeId) {
            ...
            $isDeleteStoreTitle = (bool)$result->getData('is_delete_store_bss_description_option');
            if ($existInCurrentStore) {
                if ($isDeleteStoreTitle && (int)$storeId != Store::DEFAULT_STORE_ID) {
                    ...
                    $data = $this->_prepareDataForTable(
                        new \Magento\Framework\DataObject(['bss_description_option' => $result->getBssDescriptionOption()]),
                        $titleTableName
                    );
                    $connection->update(
                        $titleTableName,
                    ...
                    $data = $this->_prepareDataForTable(
                        new \Magento\Framework\DataObject(
                            [
                                'option_id' => $result->getId(),
                                'store_id' => $storeId,
                                'bss_description_option' => $result->getBssDescriptionOption(),
                            ]
                        ),
                        $titleTableName
                    );
