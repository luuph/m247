...
class SaleOrderPlaceAfter implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        if ($this->moduleConfig->allowQtyReport()) {
            ...
            $checkOrderExist = $this->optionQtyReportFactory->create()
                ->getCollection()->addFieldToFilter('order_id', ['eq' => $order->getId()]);
            if ($checkOrderExist->getSize() < 1) {
                $data = $this->optionStockHelper->getDataQtyOption($order);
                if (!empty($data)) {
