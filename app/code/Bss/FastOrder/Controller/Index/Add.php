...
use Bss\FastOrder\Helper\HelperAdd;
...
class Add extends \Magento\Checkout\Controller\Cart\Add
{
    ...
    public function execute()
    {
        ...
        if ($catalogPermissionsPreventProductNames) {
            if (count($productNames) > HelperAdd::MAX_PRODUCTS_NAME_MESSAGE_ERROR) {
                $this->messageManager->addErrorMessage(
                    __(
                        "We can't add %1 products to cart because you have no permission to see its.",
                        count($catalogPermissionsPreventProductNames)
                    )
                );
            } else {
                $this->messageManager->addErrorMessage(
    ...
    protected function saveCart($success, $productNames, &$result)
    {
        if ($success) {
            $this->cart->save();
            $result['status'] = true;
            if (count($productNames) > HelperAdd::MAX_PRODUCTS_NAME_MESSAGE_SUCCESS) {
                $message = __(
