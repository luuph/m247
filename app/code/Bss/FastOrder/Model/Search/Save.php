...
class Save
{
    ...
    private function getShowPopup($product)
    {
        ...
        if ($product->getBssHidePrice() && !$product->getIsActiveRequest4QuoteProductPage()) {
            return 0;
        }
    ...
    public function _getProductData($product, $includePopupHtml = false, $image = 'category_page_grid')
    {
        ...
        $product->setData('product_request_4_quote', $product->getIsActiveRequest4Quote() ? 1 : 0);
    ...
    public function getStandardProductCollection($skuData, $includePopupHtml = false)
    {
        ...
        $isShowOutOfStock = $this->helperData->getDisplayOutOfStock();
        if (!$isShowOutOfStock) {
