...
define([
    ...
    $.widget('bss.FastOrderSwatch', {
        ...
        _CalcProducts: function ($skipAttributeId) {
            var $widget = this,
                selectedOptions = '.' + $widget.options.classes.attributeClass + '[bss-option-selected]',
                products = [];
            ...
            $.each(this.options.jsonConfig.attributes, function () {
                ...
                if ($widget.options.jsonConfig.canDisplayShowOutOfStockStatus) {
                    let salableProducts = $widget.options.jsonConfig.salable[item.id],
                        swatchOptions = $(container).find(`[bss-attribute-id='${item.id}']`).find('.bss-swatch-option');

                    swatchOptions.each(function (key, value) {
                        let optionId = $(value).attr('bss-option-id');
