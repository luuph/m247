...
define(
    ...
    $.widget('bss.fastorder', {
        ...
        _reloadTotalPrice: function (el, fomatPrice) {
            ...
            if (productId && qty > 0) {
                ...
                if ($(el).closest('tr.bss-fastorder-row').find('.bss-product-hide-price').val() != '1'
                    || $(el).closest('tr.bss-fastorder-row').find('.bss-product-request-4-quote').val() == '1') {
                    if (productType === "grouped") {
        ...
        addRowReplaceParent: function () {
            //add row after hide Parent configurable product
            var lineCurrent = $('#bss-fastorder-form .bss-row').filter(function () {
                return $(this).css('display') !== 'none'
            }).length
            var lineDefault = parseInt(this.options.row) + 1;
            var surLine = lineDefault - lineCurrent;
            var i;
            // add number of rows if available row less than default setting rows
            if (surLine > 0) {
                for (i = 0; i < surLine; i++) {
                    $('#bss-fastorder-form .bss-addline').click();
                }
        ...
        selectConfigurableAttributes: function (attributeData, sortOrder) {
            ...
            var isEnableRequest4Quote = parseInt(rowSelector.find('.bss-fastorder-autocomplete .bss-row-suggest .bss-product-request-4-quote').val());
            // enable open popup
            if (isHidePriceValue !== 1 || isEnableRequest4Quote === 1) {
                    ...
                    if (res.hasOwnProperty(key)) {
                        ...
                        $('#bss-fastorder-form .bss-row').each(
                            function () {
                                ...
                                if(productData['qty_increments'] && parseFloat(productData['qty_increments']) > 0) {
                                    qty_increments = productData['qty_increments'];
                                }
                                if(0 < qty && qty < qty_increments) {
                                    return true;
                                }
                                ...
                                rowSelector.find('.bss-fastorder-row-qty .bss-product-request-4-quote').val(productData['product_request_4_quote']);
                                ...
                                rowSelector.find('.bss-fastorder-autocomplete .bss-product-qty').val(parseInt(qty));
