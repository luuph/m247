...
class MinmaxAmount extends AbstractFieldArray
{
    /**
     * @var array
     */
    ...
    protected function _getCategoryRenderer()
    {
        if (!$this->_categoryRenderer) {
            $this->_categoryRenderer = $this->getLayout()->createBlock(
                \Bss\MinMaxAmountOrderPerCate\Block\Adminhtml\System\Config\Form\Category::class,
                '',
    ...
    protected function _getCustomerGroupRenderer()
    {
        if (!$this->_customerGroupRenderer) {
            $this->_customerGroupRenderer = $this->getLayout()->createBlock(
                \Bss\MinMaxAmountOrderPerCate\Block\Adminhtml\System\Config\Form\Customergroup::class,
                '',
                ['data' => ['is_render_to_js_template' => true]]
            );
    ...
    protected function _prepareToRender()
    {
        ...
        $this->addColumn('min_sale_amount', ['label' => __('Min Amount')]);
        $this->addColumn('max_sale_amount', ['label' => __('Max Amount')]);
        $this->_addAfter = false;
        $this->_addButtonLabel = __('Add Amount');
    }
    ...
    protected function _prepareArrayRow(DataObject $row)
    {
        ...
        $optionExtraAttr['option_'.$this->_getCustomerGroupRenderer()
            ...
            ='selected="selected"';
        $optionExtraAttr['option_' . $this->_getCategoryRenderer()
    ...
    {
        if ($columnName == "min_sale_amount" || $columnName == "max_sale_amount") {
