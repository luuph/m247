...
abstract class InlineEditor extends \Magento\Backend\App\Action
{
    ...
    protected function updateInventory()
    {
        ...
        $this->_eventManager->dispatch('bss_inline_editor_update_before', ['products' => $productIds]);
        ...
        foreach ($productIds as $productId) {
            ...
            if (isset($data['qty']) && $data['qty'] !='') {
                ...
                $this->saveStock($stockItemRepository, $stockItemDo);
                $updates[] = $productId;
            }
        }
        $this->_eventManager->dispatch('bss_inline_editor_update_after', ['products' => $updates]);
    }

    /**
     * @param \Magento\CatalogInventory\Api\StockItemRepositoryInterface $stockItemRepository
     * @param \Magento\CatalogInventory\Api\Data\StockItemInterface $stockItemDo
     * @return \Magento\CatalogInventory\Api\Data\StockItemInterface
     */
    protected function saveStock($stockItemRepository, $stockItemDo)
    {
     ...
     */
    protected function saveSku($product, $newSku)
    {
    ...
    protected function getAttributesData()
    {
        $postItems = $this->getRequest()->getParam('items', []);
        $productIds = array_keys($postItems);
        $productsAttributesData = $attributesData = [];
        foreach ($productIds as $productId) {
            ...
            $productsAttributesData[$productId] = $attributesData;
        }
        return $productsAttributesData;
    }
