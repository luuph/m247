...
class History extends \Magento\Framework\View\Element\Template
{
    ...
    /**
     * Get order by reserved order id
     *
     * @param object $quote
     * @return \Magento\Sales\Api\Data\OrderInterface
     */
    public function getOrderByReservedOrderId($quote)
    {
        if ($quote && $quote->getId()){
            try {
                $reservedOrderId = $quote->getReservedOrderId();
                return $this->orderFactory->create()->loadByIncrementId($reservedOrderId);
            } catch (\Exception $ex) {
                return null;
            }
        } else {
    ...
    public function getOrder($quoteExtension){
        $order = $this->getOrderByReservedOrderId($this->getQuote($quoteExtension->getTargetQuote()));
        if ($order && !$order->getEntityId()) {
            $order = $this->getOrderByReservedOrderId($this->getQuote($quoteExtension->getBackendQuoteId()));
        }
        return $order;
    }

    /**
     * Get Request Order View Url
     *
     * @param object $quote
     * @return string
     */
