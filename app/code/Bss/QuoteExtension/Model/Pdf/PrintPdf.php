...
class PrintPdf extends AbstractPdf
{
    ...
    protected function insertQuote(&$page, $quote, $requestQuote)
    {
        ...
        $orderIncrementId = $orderFactory->loadByAttribute('quote_id', $requestQuote->getTargetQuote())
            ->getIncrementId();
        if (!$orderIncrementId) {
            $orderIncrementId = $orderFactory->loadByAttribute('quote_id', $requestQuote->getBackendQuoteId())
                ->getIncrementId();
        }
        $y2 = $orderIncrementId ? ($top - 70) : ($top - 55);
