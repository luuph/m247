        ...
        $.widget(
            'mage.reorderProduct', {
                ...
                _RenderTable: function () {
                    ...
                    $('#reorder_product').on(
                        'draw.dt', function () {
                            var checkedList = {};
                            $.each(
                                $widget.options.favs, function (k, fav) {
                                    if (fav.checked) {
                                        checkedList[fav.id] = true;
                                    }
                                }
                            );
                            $(this).find('input.reorder-select-item').each(
                                function () {
                    ...
                    $("#searchbox-reorder").on(
                        "keyup search input paste cut", function () {
                ...
                _EventListener: function () {
                    ...
                    $(document).on(
                        'click', '#reorder-select-all', function () {
                            return $widget._OnClick($(this));
                        }
                    );

                    $(document).on(
                        'click', '.reorder-select-item', function () {
                            return $widget._OnClickSingle($(this));
                        }
                    );

                    $(document).on(
                        'click', '.reorder-quickview', function () {
                            return $widget._QuickView($(this));
                        }
                    );

                    $(document).on(
                        'input', '.qty-reoderproduct', function () {
                            return $widget._InputChange($(this));
                        }
                    );

                    $('body').on(
                        'click','#add-all-to-cart-reorder,#add-all-to-wishlist-reorder,.bt-reoderproduct',function (e) {
                            e.preventDefault();
