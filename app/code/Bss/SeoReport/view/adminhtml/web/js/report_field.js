    ...
    $.widget('bss.report_field', {
        ...
        handleDescription: function(){
            ...
            if (!description || description === '') {
                ...
                let descriptionLinks = $(this.htmlDecode(description)).find('a').map(function() {
            ...
            if (!mainKeyword || mainKeyword === '' || !description || description === '') {
                ...
                this.config.auditSEO.keywordFirstParagraphCount = this.checkKeywordFirstParagraph(descriptionOriginal, mainKeywordLower);
        ...
        checkKeywordFirstParagraph: function (descriptionOriginal, mainKeywordLower) {
            const regex = /<p>.*?<\/p>/s;
            if (descriptionOriginal !== undefined && descriptionOriginal !== '') {
                descriptionOriginal = this.htmlDecode(descriptionOriginal);
                const firstParagraphMatch = descriptionOriginal.match(regex);
                if (firstParagraphMatch && mainKeywordLower) {
                    const firstParagraph = firstParagraphMatch[0];
                    // Find mainKeywordLower in the first paragraph
                    const searchRegex = new RegExp(`\\b${mainKeywordLower}\\b`, "gi");
                    const match = firstParagraph.match(searchRegex);
                    if (match) {
                        return true;
                    }
                }
        ...
        htmlDecode: function (description) {
        ...
        handleAuditSEO: function () {
            ...
            if (this.config.auditSEO.keywordFirstParagraphCount) {
