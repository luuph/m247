...
class ConfigurableControl extends \Magento\Swatches\Block\Product\Renderer\Configurable
{
        ...
        foreach ($options as $attributeOption) {
            ...
            if (isset($config[$attribute->getAttributeId()][$optionId])) {
                if (!empty($config['canDisplayShowOutOfStockStatus'])) {
                    $productIds = $config['salable'][$attribute->getAttributeId()][$optionId] ?? [];
                } else {
                    $productIds = $config[$attribute->getAttributeId()][$optionId] ?? [];
                }
    ...
    /**
     * Check is enable Facebook Pixel module
     *
     * @return bool
     */
    public function checkIsEnableFaceBookPixel()
    {
        return $this->moduleConfig->isEnableFaceBookPixel();
    }
     ...
     */
    public function isEnableConfigSku()
    {
        return $this->moduleConfig->isShowSku();
    }
     ...
     * @throws \Magento\Framework\Exception\NoSuchEntityException
     */
    public function isEnableConfigName()
    {
        return $this->moduleConfig->isShowName();
    }
}
