...
class ProductData extends AbstractHelper
{
    ...
    /**
     * Ex. $this->configSdcpByProductId[product_id]
     *
     * @var array
     */
    protected $configSdcpByProductId = [];
     ...
     * @param \Magento\Review\Model\ResourceModel\Review\Summary\CollectionFactory $summaryColFactory
     */
    public function __construct(
        ...
        \Magento\Review\Model\ResourceModel\Review\Summary\CollectionFactory       $summaryColFactory
    ) {
        ...
        $this->summaryColFactory = $summaryColFactory;
    ...
    public function getAllProductName($productEntityId)
    {
        ...
        $summaryParent = $this->getCollectionSummary($product);
        $reviewsParent = $this->getCollectionReviewSize($product);
        ...
        foreach ($parentProduct as $simpleProduct) {
            ...
            $result['child'][$key] = [
                ...
                'summary' => $this->getCollectionSummary($simpleProduct),
                'reviews' => $this->getCollectionReviewSize($simpleProduct)
            ];
        }

        $productEnableSdcp = $this->getEnabledModuleOnProduct($productEntityId)['enabled'];
        $result['child']['default'] = [
            'name' => $product->getName(),
            'summary' => $summaryParent,
            'reviews' => $reviewsParent,
            'enable_sdcp' => (int)$productEnableSdcp // Check enable module on this product
        ];
        return $result;
    }
    ...
    public function getAllData($productEntityId)
    {
        ...
        $result['suffix'] = $this->moduleConfig->getSuffix();
        if ($result['url'] == null) {
    ...
    /**
     * Get summary collection
     *
     * @param \Magento\Catalog\Model\Product $product
     * @return int
     */
    public function getCollectionSummary($product)
    {
        $collection = $this->summaryColFactory->create()->addStoreFilter(
            $this->moduleConfig->getStoreManager()->getStore()->getId()
        )->addEntityFilter(
            $product->getId()
        );
        $summary = $collection->getFirstItem();
        return (int)$summary['rating_summary'];
    }
...
//              $detailStock = $this->salesChannel->getStockDetail($stockId, $data['sku']);
                $data['stock_number'] = 0;
                $stockStatus = false;
                if (!$childStock->getManageStock() && !$childStock->getUseConfigManageStock()) {
                    $stockStatus = 99999;
                }
    ...
    {
        if (!isset($this->configSdcpByProductId[$productId])) {
            $resultObject = $this->productEnabledModule->create()->load($productId);
            if (!$resultObject->getProductId()) {
                $this->configSdcpByProductId[$productId] = $this->productEnabledModule->create()->setData(['enabled' => 1, 'is_ajax_load' => 0]);
            } else {
                $this->configSdcpByProductId[$productId] = $resultObject;
            }
        }

        return $this->configSdcpByProductId[$productId];
    }
