...
class UrlIdentifier extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {
        return $this->getRewriteModelUrl($urlKey);
    }

    /**
     * Get rewrite model url
     *
     * @param string $urlKey
     * @return \Magento\Framework\DataObject|\Magento\UrlRewrite\Model\UrlRewrite
     * @throws NoSuchEntityException
     */
    public function getRewriteModelUrl($urlKey)
    {
        if (isset($this->rewriteModelUrl[$urlKey])) {
            return $this->rewriteModelUrl[$urlKey];
        } else {
            $urlKeyFilter = $urlKey;
            if ($this->moduleConfig->getSuffix() && strpos($urlKey, $this->moduleConfig->getSuffix()) === false) {
                $urlKeyFilter .= $this->moduleConfig->getSuffix();
            }
            $this->rewriteModelUrl[$urlKey] = $this->urlRewriteFactory->create()->getCollection()
                ->addFieldToFilter('entity_type', 'product')
                ->addFieldToFilter('request_path', ['like' => $urlKeyFilter])
    ...
    {
        if ($this->moduleConfig->getSuffix() != null && strpos($url, $this->moduleConfig->getSuffix()) !== false) {
            $url = substr($url, 0, strpos($url, $this->moduleConfig->getSuffix()));
        }
        ...
        foreach ($superData as $datas) {
            ...
            $value = str_replace('*', ',', $value);
            if (isset($map) && isset($map2) && array_key_exists($code, $map) && array_key_exists($map[$code], $map2)) {
                if (isset($map[$code]) && $map[$code] != '' && isset($map2[$map[$code]][$value])) {
