...
class ProductInitObserver implements ObserverInterface
{
     ...
     *
     * @param UrlIdentifier $urlIdentifier
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        if ($this->moduleconfig->isModuleEnable()){
            ...
            if ($this->validateObserver($request, $product)) {
                try {
                    if ($this->productData->getEnabledModuleOnProduct($product->getId())['enabled']) {
                        ...
                        $this->replaceImageByPreselect($product, $pathInfo);
                    }
                } catch (\Exception $e) {
     ...
     * @throws \Magento\Framework\Exception\NoSuchEntityException
     */
    private function replaceImageByPreselect($product, $pathInfo = null)
    {
