...
define([
    ...
    return function (SwatchRenderer) {
        $.widget('mage.SwatchRenderer', SwatchRenderer, {
            ...
            _init: function() {
                this._super();
                if (this.options.jsonChildProduct) {
                    var nameParent = this.options.jsonChildProduct.name;
                    this._UpdateReviewName(nameParent);
                }

                if (this.options.jsonModuleConfig && this.options.jsonModuleConfig.cpwd) {
                ...
                $(document).ready(function () {
                    if (self.options.jsonConfig.productStockAlert !== undefined) {
                        $(document).ajaxComplete(function (event, xhr, settings) {
                            if (settings.url.includes('/productstockalert/ajax/formData')) {
                                self._UpdateDetailStockForm($(this).find('.swatch-option.selected'));
                            }
                        });
                    }
                });
            ...
            _RenderControls: function () {
                if (this.options.jsonModuleConfig) {
            ...
            getDataofAttr: function(element, name) {
