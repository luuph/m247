...
class ExportPost extends \CommerceExtensions\CategoriesImportExport\Controller\Adminhtml\Data
{
	...
	*/
    ...
    public function execute()
    {
		...
		$ProductMetadataInterface = $this->_objectManager->create('Magento\Framework\App\ProductMetadataInterface');
        ...
        $headers = new \Magento\Framework\DataObject(
            [
                ...
                'thumbnail' => __('category_thumb_image'),
		...
		*/
        foreach ($this->_objectManager->create(
           ...
           // $template .= ',"{{' . $storeTitle . '}}"';
            ...
            $storeTemplate['store'] = str_replace("default","admin",$store->getCode());
			$storeId = $store->getId();
			...
			foreach ($childrenCategories as $categoryId) {
				...
				if(!in_array($categoryId, $_allRootIds)) {
					...
					for ($i=2, $l=sizeof($pathArr); $i<$l; $i++) {
					   if($pathArr[$i] != "") {
								if(!is_object($collection->getItemById($pathArr[$i]))){
									$logger->log(100,"No category found for ". $pathArr[$i]);
									continue;
								}
					...
					if($params['export_full_image_paths'] == "true") {
						...
						if($ProductMetadataInterface->getEdition() == "Enterprise") {
							if($category->getData('thumbnail')!="") {
								$storeTemplate['thumbnail'] = $getBaseUrl . "catalog/category/" . $category->getData('thumbnail'); 
							} 
						}
					}
					...
					$storeTemplate['available_sort_by'] = "";
					if(is_array($category->getData('available_sort_by'))) {
						$storeTemplate['available_sort_by'] = implode(",", $category->getData('available_sort_by'));
					}
					$storeTemplate['default_sort_by'] = $category->getData('default_sort_by');
