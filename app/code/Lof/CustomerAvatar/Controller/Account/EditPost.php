...
namespace Lof\CustomerAvatar\Controller\Account;
...
class EditPost extends AbstractAccount implements CsrfAwareActionInterface, HttpPostActionInterface
{
    ...
    public function execute()
    {
        ...
        if ($validFormKey && $this->getRequest()->isPost()) {
            ...
            //setting customer DOB for Arabic Store
            $dob = $this->_request->getParam('dob');
            if ($dob) {
                $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
                $storeManager = $objectManager->get(\Magento\Store\Model\StoreManagerInterface::class);
                $locale = $storeManager->getStore()->getCode();
                if (strpos($locale, 'ar') !== false) {
                    $parsed = \DateTime::createFromFormat('d/m/Y', $dob);
                    if ($parsed) {
            ...
            try {
                ...
                if ($dob) {
                    $customerCandidate->setDob($dob); // Set the corrected DOB value
                }

                $updatedCustomer = $this->customerRepository->getById($customerCandidate->getId());
                $customermobilenumobj=$updatedCustomer->getCustomAttribute("mobilenumber");
                $customermobinumber=$customermobilenumobj ? $customermobilenumobj->getValue() : null;
                $customerCandidate->setCustomAttribute('mobilenumber', $customermobinumber);
