...
{
    public const ENTITY_CODE = 'mp_directory_city';

    public const TABLE_MAIN = 'directory_country_region_city';
    public const TABLE_LOCALE = 'directory_country_region_city_name';
    ...
    public const COL_DEFAULT_NAME = 'default_name';
    public const COL_LOCALE = 'locale';
    public const COL_LOCALE_NAME = 'name';

    public const LOCALE_PREFIX = 'name';

    public const VALIDATOR_REGION = 'validator_region';
    ...
    protected function prepareEntityRow($rowData)
    {
        ...
        foreach ($rowData as $field => $value) {
            if (in_array($field, $this->getMainColumns())) {
                $preparedData['main'][$field] = trim((string) $value);
            }
            ...
            if (preg_match('/' . self::LOCALE_PREFIX . ':(.*)/', $field, $matches)) {
                ...
                if (empty($locale) || ! strlen((string) $value)) {
                    continue;
                }

                $preparedData['locale'][$locale] = trim((string) $value);
            }
        }
        return $preparedData;
    }
