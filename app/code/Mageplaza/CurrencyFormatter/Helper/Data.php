...
class Data extends AbstractData
{
    ...
    public function getDirectoryCurrency($result, $decimal, $original, $config)
    {
        ...
        $originDecimals  = explode($original['decimalSymbol'], $result);
        $beforeSeparator = $originDecimals[0];
        $afterSeparator  = $originDecimals[1] ?? '';
        if ($decimal === 0) {
            return $this->processShowSymbol($config['symbol'], $beforeSeparator, $config['show_symbol'], $negative);
        }
        $afterSeparator = $afterSeparator . '0000';
        $thousandsArr   = explode(',', $beforeSeparator);
        $thousands      = '';
        if (count($thousandsArr) >= 2) {
            $idFirst = false;
            foreach ($thousandsArr as $thousand) {
                $groupSeparator = $idFirst ? $config['group_separator'] : '';
                $idFirst        = true;
                $thousands      .= $groupSeparator . $thousand;
            }
        } else {
            $thousands = $thousandsArr[0];
        }

        $afterSeparator = substr($afterSeparator, 0, $decimal);
        if ($afterSeparator !== '') {
            $result = $thousands . $config['decimal_separator'] . $afterSeparator;
        } else {
            $result = $thousands;
        }
