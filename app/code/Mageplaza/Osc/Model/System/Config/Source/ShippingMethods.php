        ...
        foreach ($carrierMethodsList as $carrierMethodCode => $carrierModel) {
            if ($carrierModel->getAllowedMethods()) {
                foreach ($carrierModel->getAllowedMethods() as $shippingMethodCode => $shippingMethodTitle) {
                    ...
                    $shippingMethodCode = is_string($shippingMethodCode) ? $shippingMethodCode : json_encode($shippingMethodCode);
                    $shippingMethodTitle = is_string($shippingMethodTitle) ? $shippingMethodTitle : json_encode($shippingMethodTitle);
