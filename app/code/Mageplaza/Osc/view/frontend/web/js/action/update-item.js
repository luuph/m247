...
define([
    ...
    return function (payload) {
        ...
        if (loadingSpeedConfig && oscConfig && loadingSpeedConfig.refresh_page !== '1') {
            itemUpdateLoader = ['total'];
        }

        if ( loadingSpeedConfig && oscConfig  && loadingSpeedConfig.refresh_page !== '1'
            && loadingSpeedConfig.product_change.includes("1")) {
            itemUpdateLoader = ['shipping', 'total'];
        }
        ...
        return storage.post(
            ...
            function (response) {
                ...
                } else if (oscConfig && window.loadingSpeedConfig.product_change.includes("1")
                    && response.shipping_methods && !quote.isVirtual()) {
                    shippingService.setShippingRates(response.shipping_methods);
                }
