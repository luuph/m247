...
class Gird extends Action
{
    ...
    public function execute()
    {
        ...
        return $resultRaw->setContents($productListHtml . $this->getScriptHtml() . $this->getShowMessageScriptHtml());
    }
...
SCRIPT;
     ...
     * @return string
     */
    public function getShowMessageScriptHtml()
    {
        return <<<SCRIPT
        <script type="text/javascript">
            require(['jquery'], function($) {
                $(document).ready(function() {
                    const listBrandProductsElement = document.getElementById("list-brand-products");

                    const toggleNoteVisibility = function() {
                        const selectedCount = parseInt($("#product_grid_massaction-count strong[data-role='counter']").text());
                        if (selectedCount > 1000) {
                            $(".list-brand-products-note").show();
                        } else {
                            $(".list-brand-products-note").hide();
                        }
                    };
                    if (listBrandProductsElement) {
                        const observer = new MutationObserver(function() {
                            toggleNoteVisibility();
                        });
                        observer.observe(listBrandProductsElement, {
                            childList: true, 
                            subtree: true     
                        });
                    }
