...
class ListProduct extends Template
{
    ...
    public function aroundGetProductPrice(
        ...
        $brandObj = $this->getBrandObject($product) ?: '';
        if ($product->getTypeId() === 'configurable' && $brandObj === '') {
            $children = $product->getTypeInstance()->getUsedProducts($product);
            foreach ($children as $child) {
                if ($this->getBrandObject($child)) {
                    $brandObj = $this->getBrandObject($child);
                    break;
                }
            }
        }
        ...
        $info = $this->getLayout()
            ...
            ->setTemplate('Mageplaza_Shopbybrand::product/list/logo.phtml')
            ->setIsShowList(1)
            ->setBrand($brandObj)
            ->toHtml();
     ...
     */
    public function getBrandObject($product)
    {
        if (!$product instanceof Product) {
            return null;
        }

        if (!$optionId = $product->getData($this->helper->getAttributeCode())) {
            return null;
        }
