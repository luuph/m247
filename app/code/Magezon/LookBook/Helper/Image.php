...
class Image extends AbstractHelper implements ArgumentInterface
{
    ...
    public function init($product, $imageId, $attributes = [])
    {
        ...
        $this->attributes = array_merge(
            $this->getConfigView()->getMediaAttributes('Magezon_LookBook', self::MEDIA_TYPE_CONFIG_NODE, $imageId),
            $attributes
        );
    ...
    {
        if ($placeholder) {
            $placeholderFullPath = 'Magezon_LookBook::images/product/placeholder/' . $placeholder . '.jpg';
        } else {
            $placeholderFullPath = $this->_placeholder
                ?: 'Magezon_LookBook::images/product/placeholder/' . $this->_getModel()->getDestinationSubdir() . '.jpg';
        }
    ...
    public function getFrame()
    {
        ...
        if ($frame === null) {
            $frame = $this->getConfigView()->getVarValue('Magezon_LookBook', 'product_image_white_borders');
        }
        return (bool)$frame;
    }
