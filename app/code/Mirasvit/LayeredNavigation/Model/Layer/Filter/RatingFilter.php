...
class RatingFilter extends AbstractFilter
{
    ...
    {
        $isApplied = $this->configProvider->isPreCalculationEnabled() ? false : $this->isApplied;

        if (
            !$this->extraFilterConfigProvider->isRatingFilterEnabled() 
            || $isApplied
        ...
        foreach ($this->ratingValues as $ratingValue) {
            ...
            if (
                $count === 0 
                || ($this->configProvider->isHideUnusefulOptionEnabled() && ($size == $cumulativeCount))
            ) {
                continue;
            }
