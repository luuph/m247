...
class Shipping extends AbstractCarrier implements CarrierInterface
{
    ...
    public function collectRates(RateRequest $request)
    {
        ...
        try {
            ...
            $currencyRate = $mfListObj->getCurrencyRate($storeCurrency);
            ...
            foreach ($availableMethods as $id) {
                ...
                $data = $sMFObj->calculateShippingCharge($curlData);

                //convert the amount from portal currency to store current currency
                $displayAmount = ((int) ($data->Fees * $currencyRate * 1000)) / 1000;

                //convert the amount from store current currency to store base currency
                $storeBaseAmount = $displayAmount / $storeRate;

                $rateResult->append($this->createShippingMethod($storeBaseAmount, $id));
            }
            return $rateResult;
