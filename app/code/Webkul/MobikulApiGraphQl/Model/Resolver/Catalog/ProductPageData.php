...
/**
 * ProductPageData resolver
 */
class ProductPageData extends AbstractCatalog implements ResolverInterface
{
    /**
     * Sku variable
     *
    ...
    /**
     * IsIncludeTaxInPrice variable
     *
     * @var boolean
     */
    protected $isIncludeTaxInPrice;
    ...
    public function resolve(
        ...
        try {
            ...
            $cacheString = "PRODUCTPAGEDATA".$this->productId.$this->width.$this->quoteId.
            $this->storeId.$this->customerToken.$this->sku.$this->currency;
            if ($this->isCacheEnabled()) {
                $categoryCacheIdentifier = $cacheString;
                $productPage = $this->getDecodedFromCache($categoryCacheIdentifier);
                if (!empty($productPage)) {
                    if ($this->customerId > 0) {
                        $productPage['wishlistItems'] = [];
            ...
            $currency = $this->currency ?? $this->store->getBaseCurrencyCode();
            ...
            if ($this->customerId != 0) {
                ...
                $this->returnArray["cartCount"] = (int) $this->helper->getCartCount($customerQuote);
            ...
            $this->coreRegistry->register("current_product", $this->product);
            if ($this->defaultStockQty->isMsgVisible()) {
                $this->returnArray["isThresholdVisible"] = true;
                $this->returnArray["thresholdQtyLeft"] = $this->defaultStockQty->getStockQtyLeft();
            } else {
                $this->returnArray["isThresholdVisible"] = true;
            }
    ...
    protected function getRatingAndReviewData()
    {
        ...
        foreach ($ratingCollection as $rating) {
            if ($rating->getSummary()) {
                ...
                $eachRating["ratingValue"] = number_format((float)(5 * $rating->getSummary()) / 100, 1, ".", "");
        ...
        foreach ($reviewCollection as $review) {
            ...
            if (count($votes)) {
                foreach ($votes as $_vote) {
                    ...
                    $oneVote["value"] = number_format((float)$_vote->getValue(), 1, ".", "");
                    $totalRatings += number_format((float)$_vote->getValue(), 1, ".", "");
                    $totalRatingsCount++;
                    $ratings[] = $oneVote;
            ...
            $oneReview["avgRatings"] = number_format(
                (float)($totalRatings/($totalRatingsCount ? $totalRatingsCount: 1)),
                1,
                ".",
                ""
            );
        ...
        if (count($ratingsArr) > 0) {
            $ratingVal = number_format((float)(5 * (array_sum($ratingsArr) / count($ratingsArr))) / 100, 1, ".", "");
        }
        $this->returnArray["rating"] = (float)$ratingVal;
    }
    ...
    protected function getProductBasicDeatils()
    {
         ...
         // Get the brand option ID from the product
    $brandOptionId = $this->product->getData('brand');
    $brandLabel = null;

    // Fetch the label for the brand option ID
        ...
        $this->returnArray['brand_name'] = $brandLabel ?? null; // Set label or null if not available

        $this->returnArray['brand'] = $this->product->getData('brand');
    $this->returnArray['more_details_product_tab'] = $this->product->getData('more_details_product_tab');
    $this->returnArray['delivery_product_tab'] = $this->product->getData('delivery_product_tab');
   
        $this->returnArray["id"] = $this->productId;
        $this->returnArray["name"] = htmlspecialchars_decode($this->product->getName() ?? '');
        $this->returnArray["sku"] = $this->product->getSku();
        ...
        if ($this->isIncludeTaxInPrice) {
            ...
            $this->returnArray["specialPrice"] = $this->taxHelper->getTaxPrice(
                $this->product,
                $this->pricingHelper->currency($this->product->getSpecialPrice(),false,false)
            );
            ...
            $this->returnArray["formattedSpecialPrice"] = $this->helperCatalog->stripTags($this->priceCurrency->format(
                $this->taxHelper->getTaxPrice($this->product, $this->pricingHelper->currency($this->product->getSpecialPrice(),false,false))
            ));
        } else {
            $this->returnArray["price"] = $price;
            $this->returnArray["finalPrice"] = $finalPrice;
            $this->returnArray["specialPrice"] = $this->helperCatalog->stripTags($this->pricingHelper->currency($this->product->getSpecialPrice(),false,false));
            $this->returnArray["formattedPrice"] = $this->helperCatalog->stripTags(
            ...
            $this->returnArray["formattedSpecialPrice"] = $this->helperCatalog->stripTags(
                $this->priceCurrency->format($this->pricingHelper->currency($this->product->getSpecialPrice(),false,false))
            );
        }
