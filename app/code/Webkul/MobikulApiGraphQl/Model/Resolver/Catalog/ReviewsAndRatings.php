...
/**
 * ReviewsAndRatings resolver
 */
class ReviewsAndRatings extends AbstractCatalog implements ResolverInterface
{
    /**
     * @inheritdoc
     */
            ...
            foreach ($reviewCollection as $review) {
                ...
                if (count($votes)) {
                    foreach ($votes as $_vote) {
                        ...
                        $oneVote["value"] = number_format((float)$_vote->getValue(), 2, ".", "");
                        $totalRatings += number_format((float)$_vote->getValue(), 2, ".", "");
                        $totalRatingsCount++;
                        $ratings[] = $oneVote;
                        $ratingsArr[] = $_vote->getPercent();
                    }
                }
                $oneReview["avgRatings"] = $totalRatingsCount ? round(
