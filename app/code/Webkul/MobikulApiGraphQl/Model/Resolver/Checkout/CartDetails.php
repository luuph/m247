...
/**
 * CartDetails resolver
 */
class CartDetails extends AbstractCheckout implements ResolverInterface
{
    /**
     * Currency variable
     *
    ...
    public function resolve(
        ...
        try {
            ...
            if ($this->customerId != 0) {
                $this->quote = $this->helper->getCustomerQuote($this->customerId)->setStoreId($this->storeId);
            }
            if ($this->quoteId != 0) {
            ...
            $this->returnArray['quoteId'] = $this->quote->getId();
            if ($this->customerId != 0 || $this->quoteId != 0) {
                ...
                foreach ($cartItemData["items"] as $cartData) {
                    if ($this->customerId == 0) {
                        if ($cartData['typeId'] == "downloadable") {
                            $this->returnArray["descriptionMessage"] = __("please login to proceed with checkout");
                            $this->returnArray["isCheckoutAllowed"] = false;
                        }
                    }
                }
            ...
            if ($product->getTypeId() == "bundle") {
                ...
                foreach ($bundleOptions as $bundleOption) {
                    ...
                    foreach ($bundleOption["value"] as $bundleOptionValue) {
                        ...
                        $eachBundleOption["valueIds"][] = $bundleOptionValue["qty"];
    ...
    {
        foreach ($this->quote->getMessages() as $key => $msg) {
            $this->returnArray["descriptionMessage"] = $msg->getText();
            if ($key == "qty") {
                foreach ($this->quote->getItemsCollection() as $quote_item) {
                    if ($quote_item->getHasError()) {
                        $this->returnArray["descriptionMessage"] = __(
                            "The requested quantity of %1 is not available",
                            $quote_item->getName()
                        );
                    }
                }
