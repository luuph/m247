 ...
 * Login resolver
 */
class Login extends AbstractCustomer implements ResolverInterface
{
    /**
     * @inheritdoc
     */
    public function resolve(
        ...
        try {
            ...
            if ($this->customer->getId() > 0) {
                ...
                $this->returnArray["mobile"] = $this->customer->getCustomAttribute('mobilenumber') 
    ? $this->customer->getCustomAttribute('mobilenumber')->getValue() 
    : '';
            ...
            foreach ($collection as $value) {
                if ($value->getBanner() != "") {
                    if ($value->getIsSocial() == 1) {
                        ...
                        $basePath = $this->baseDir.DS."mobikul".DS."customerpicture".DS.$this->customerId
                        .DS.'banner'.DS.$value->getBanner();
                        $newUrl = "";
                        if ($this->fileDriver->isFile($basePath)) {
                            $newPath = $this->baseDir.DS."mobikulresized".DS.$this->width."x".$this->height.DS.
                            "customerpicture".DS.$this->customerId.DS.'banner'.DS.$value->getBanner();
                            ...
                            DS."customerpicture".DS.$this->customerId.DS.'banner'.DS.$value->getBanner();
                        }
                ...
                if ($value->getProfile() != "") {
                    if ($value->getIsSocial() == 1) {
                        ...
                        $basePath = $this->baseDir.DS."mobikul".DS."customerpicture".DS.$this->customerId
                        .DS.'profile'.DS.$value->getProfile();
                        $newUrl = "";
                        if ($this->fileDriver->isFile($basePath)) {
                            $newPath = $this->baseDir.DS."mobikulresized".DS.$this->profileWidth."x".
                            $this->profileHeight.DS."customerpicture".DS.$this->customerId.DS.'profile'
                            .DS.$value->getProfile();
    ...
    /**
     * MergeProductCompareList function
     *
     * @return void
     */
