...
class Save extends \Webkul\MobikulCore\Controller\Adminhtml\Bannerimage
{
    ...
    public function execute()
    {
        ...
        if ($originalRequestData) {
            try {
                ...
                if (!isset($bannerimageData['filename'][0]['path']) &&
                    isset($bannerimageData['filename'][0]['url'])
                ) {
                    $url = $bannerimageData['filename'][0]['url'];
                    $newUrl = explode('media/', $url);
                    $bannerimageData["filename"] = $newUrl[1];
                } else {
    ...
    private function getBannerStoreId($bannerimageData)
    {
        if (isset($bannerimageData["store_id"])) {
            if (in_array(0, $bannerimageData["store_id"])) {
