...
class Catalog extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     * @param \Magento\Catalog\Model\Layer\Search $layerSearch
     ...
     * @param \Magento\Wishlist\Model\ResourceModel\Item\CollectionFactory $wishListCollection
     ...
     * @param \Webkul\MobikulCore\Helper\Data $helperData
     */
    public function __construct(
        ...
        \Webkul\MobikulCore\Helper\Data $helperData,
     ...
     * @return array
     */
    public function getOneProductRelevantData(&$product, $storeId, $width, $customerId = 0, $categoryId = 0)
    {
        ...
        $reviews = $this->reviewModel
            ...
            ->addRateVotes();
        $eachProduct = [];
        $eachProduct['categoryId'] = $categoryId;
        ...
        if ($product->getTypeId() == "configurable") {
            ...
            $configurableBlockData = json_encode($configurableBlock->getJsonConfig());
            if($this->isJson($configurableBlockData)){
                $eachProduct["configurableData"] = json_decode($configurableBlockData);
            } else {
                $eachProduct["configurableData"] = $configurableBlockData;
            }
        } else {
        ...
        $visibilityOptions = [
            \Magento\Catalog\Model\Product\Visibility::VISIBILITY_NOT_VISIBLE => "Not Visible Individually",
            \Magento\Catalog\Model\Product\Visibility::VISIBILITY_IN_CATALOG => "Catalog",
            \Magento\Catalog\Model\Product\Visibility::VISIBILITY_IN_SEARCH  => "Search",
            \Magento\Catalog\Model\Product\Visibility::VISIBILITY_BOTH       => "Catalog, Search",
        ];
        ...
        if (!$visibility) {
            ...
            $attributeTable = $resourceConnection->getTableName('catalog_product_entity_int');
            $eavAttributeTable = $resourceConnection->getTableName('eav_attribute');

            $visibility = $connection->fetchOne(
                $connection->select()
                    ->from(['cpei' => $attributeTable], ['value'])
                    ->join(['ea' => $eavAttributeTable], 'cpei.attribute_id = ea.attribute_id', [])
                    ->where('cpei.entity_id = ?', $product->getId())
                    ->where('ea.attribute_code = ?', 'visibility')
                    ->where('cpei.store_id IN (?)', [0, $storeId]) // Global or store-specific visibility
                    ->order('cpei.store_id DESC') // Prefer store-specific value
                    ->limit(1)
            );
        }
        
        $eachProduct["visibility"] = $visibilityOptions[$visibility] ?? "Unknown";
        ...
        $eachProduct["brand"] = $product->getData('brand');

        $brandOptionId = $product->getData('brand');
        $attribute = $this->eavConfig->getAttribute(\Magento\Catalog\Model\Product::ENTITY, 'brand');
        $brandLabel = $attribute->getSource()->getOptionText($brandOptionId);
        $eachProduct["brand_name"] = $brandLabel ?? $brandOptionId; // Use label if available, otherwise fallback to ID
        $eachProduct["brand_name"] = $brandLabel ?: null;
        ...
        $eachProduct["dominantColor"] = $this->helperData->getDominantColor($eachProduct["thumbNail"]);
    ...
    protected function isProductNew($product)
    {
        ...
        if ($productNewToDate === "" && $productNewFromDate === "") {
            return false;
        }
        if (strtotime($todayStartOfDayDate) >= strtotime($productNewFromDate)
            ...
            return true;
        } elseif (strtotime($todayStartOfDayDate) >= strtotime($productNewFromDate) && $productNewToDate === null) {
            return true;
        } elseif (strtotime($todayEndOfDayDate) <= strtotime($productNewToDate) && $productNewFromDate === null) {
            return true;
        } else {
            return false;
        }
    ...
    public function getWebsiteData()
    {
        ...
        $isStoreCodeToUrlsEnabled = $this->scopeConfig->getValue("web/url/use_store", \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        $storeData = [];
        foreach ($websites as $website) {
            foreach ($website->getStores() as $store) {
                $storeObj = $this->storeManager->getStore($store);
                $storeCode = "";
                if($isStoreCodeToUrlsEnabled == 1 || $isStoreCodeToUrlsEnabled == true){
                    $storeCode = $storeObj->getCode();
                }
                $url= $storeObj->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_WEB).$storeCode;
            }
            $websiteData[] = [
    ...
    {
        try {
            $width = $width == 0 ? 1000 : (int) $width;
            $height = $height == 0 ? 1000 : (int) $height;
     ...
     *
     * @param mixed $value
     * @param mixed $optionsFacetedData
     * @return void
     */
    ...
    /**
     * IsOptionReducesResults function
     *
    ...
    /**
     * RenderPriceRangeLabel function
     *
    ...
    {
        $requestHeader = $this->requestInterface->getHeaders()->toArray() ?? [];
        ...
        } elseif ($product->getArType() == 1 && $userAgent == "iOS") {
            return $product->getArModelFileIos();
