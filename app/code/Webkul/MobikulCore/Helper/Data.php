...
use Webkul\MobikulCore\Model\Config\Backend\PaymentMethods as MobikulPaymentConfig;
...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     * Cache variable
     *
     * @var \Webkul\MobikulCore\Model\CacheFactory
     */
    protected $cache;
     ...
     * MobiToken variable
     *
     * @var \Webkul\Mobikul\Model\OauthToken
     */
    protected $mobiToken;
    
    /**
     * OauthHelper variable
     *
     * @var OauthHelper
     */
    ...
    /**
     * OauthTokenFactory variable
     *
     * @var \Webkul\Mobikul\Model\OauthTokenFactory
     */
    ...
    /**
     * MobikulPaymentConfig variable
     *
     * @var MobikulPaymentConfig
     */
    protected $mobikulPaymentConfig;
     ...
     * @param \Webkul\MobikulCore\Model\CacheFactory $cache
     * @param \Magento\Checkout\Helper\Data $checkoutHelper
     * @param \Magento\Framework\App\Helper\Context $context
     * @param \Magento\Framework\Filesystem\Driver\File $file
     * @param \Magento\Framework\Json\Helper\Data $jsonHelper
     * @param \Webkul\MobikulCore\Model\OauthToken $mobiToken
     ...
     * @param \Webkul\MobikulCore\Model\OauthTokenFactory $oauthTokenFactory
     ...
     * @param MobikulPaymentConfig $mobikulPaymentConfig
    ...
    public function __construct(
        ...
        MobikulPaymentConfig $mobikulPaymentConfig,
        Wishlist $wishlist,
        ...
        $this->mobikulPaymentConfig = $mobikulPaymentConfig;
  ...
  /**
     * Placeholder for getGuestQuote
     */
    public function getGuestQuote($quoteId)
    {
        try {
            $quote = $this->quoteFactory->create()->load($quoteId);
            if ($quote->getId() && $quote->getIsActive() && !$quote->getCustomerId()) {
                return $quote;
            }
        } catch (\Exception $e) {
    ...
    public function getConfigData($path, $scope = \Magento\Store\Model\ScopeInterface::SCOPE_STORE)
    {
        ...
        $returnValue = $this->scopeConfig->getValue($path, $scope, $storeId);
        if($path == "mobikul/mobikulpayment/selectedmethods") {
            $returnValueArray = explode(',', $returnValue ?? "");
            $returnValue = "";
            $mobikulPaymentMethods = $this->mobikulPaymentConfig->mobikulPaymentOptionsArray();
            foreach($returnValueArray as $configValue) {
                if(in_array($configValue, $mobikulPaymentMethods)) {
                    $returnValue = $returnValue == "" ? $configValue : $returnValue.",".$configValue;
                }
            }
        }
        return $returnValue;
     ...
     * @return string
     */
    public function getTokenByCustomerDetails($username, $password, $customerId, $isSocial = 0)
    {
        $token = $this->mobiToken->loadByCustomerId($customerId)->getToken();
        if (!$token && $username && ($password || $isSocial)) {
    ...
    {
        $apiName = $this->request->getFullActionName();
        $cacheStatus = (bool)$this->getConfigData("mobikul/cachesettings/enable");
        if ($cacheStatus) {
            $counter = $this->getConfigData("mobikul/cachesettings/counter");
            if ($counter == "") {
            ...
            if (count($collection)) {
                foreach ($collection as $eachEntry) {
                    ...
                    $eachEntry->setApiName(strtolower($apiName));
                    $eachEntry->setId($eachEntry->getId());
                    $eachEntry->save();
                }
            } else {
    ...
    {
        return $this->getConfigData("mobikul/configuration/dominant_color") ?? "#dfdfdf";
    }
    ...
    public function getValidDimensions($mFactor, $length)
    {
        ...
        return floor($dimension > 2500 ? 2500 : $dimension);
    }
    ...
    /**
     * Function to get purchase point
     *
     * @param mixed $value
     * @param mixed $order
     * @return void
     */
    public function setPurchasePoint($value, $order)
    {
        $orderId = $order->getEntityId();
        $orderModel = $this->orderFactory->create()->load($orderId);
        $orderModel->setPurchasePoint($value);
        $orderModel->save();
    }
     ...
     * 
     * Returns wishlist according to customer ID.
     *
     * @param int $customerId
     * @return array
     */
    public function getCustomerWishlist($customerId) {
        $customerWishlist = $this->wishlist->loadByCustomerId($customerId)
            ->getItemCollection()->getData();
        return array_column($customerWishlist, 'product_id');
    }
}